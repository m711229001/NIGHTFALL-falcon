# -*- coding: utf-8 -*-
"""Endpoint Catalog + CLI registration."""
...
print("=" * 60)
print("Endpoint Catalog Phase 1: Aggregator Module")
print("=" * 60)

# ============================================================
# FILE 1: endpoint_catalog.py
# ============================================================
catalog_content = r'''"""Falcon MAG Framework - Endpoint Catalog
...
(modules / "endpoint_catalog.py").write_text(catalog_content, encoding="utf-8")
print("[OK] endpoint_catalog.py created")
print()
...
old_reg = '''    "js_endpoints":         "modules.js_endpoints",'''
new_reg = '''    "js_endpoints":         "modules.js_endpoints",
    "endpoint_catalog":     "modules.endpoint_catalog",'''

if old_reg in cli and 'endpoint_catalog' not in cli:
    cli = cli.replace(old_reg, new_reg, 1)
    print("[OK] added to MODULE_REGISTRY")

# Add to FEEDER_MODULES (last)
old_feeders = 'FEEDER_MODULES = ["crawler", "playwright_crawler", "js_analyzer", "js_endpoints", "param_discovery"]'
new_feeders = 'FEEDER_MODULES = ["crawler", "playwright_crawler", "js_analyzer", "js_endpoints", "param_discovery", "endpoint_catalog"]'

if old_feeders in cli:
    cli = cli.replace(old_feeders, new_feeders, 1)
    print("[OK] added to FEEDER_MODULES")

# Add FEEDER_KEYS
old_keys = '''    "js_endpoints":       "_js_endpoints",
    "param_discovery":    "_param_discovery",
}'''
new_keys = '''    "js_endpoints":       "_js_endpoints",
    "param_discovery":    "_param_discovery",
...
if old_keys in cli:
    ...
    print("[OK] added to FEEDER_KEYS")

# Add to MODE_PRESETS fast
old_fast = '''            "fingerprint", "headers_check", "clickjacking",
            "cors_checker", "cookies_checker",
        ],
        "param_discovery_max": 50,'''
new_fast = '''            "fingerprint", "headers_check", "clickjacking",
            "cors_checker", "cookies_checker", "endpoint_catalog",
        ],
        "param_discovery_max": 50,'''

if old_fast in cli:
    cli = cli.replace(old_fast, new_fast, 1)
    print("[OK] added to fast mode")

# Add to normal mode
old_normal = '''            "xss_scanner", "sqli_scanner",
            "http_methods", "open_redirect",
        ],
        "param_discovery_max": 150,'''
new_normal = '''            "xss_scanner", "sqli_scanner",
            "http_methods", "open_redirect", "endpoint_catalog",
        ],
        "param_discovery_max": 150,'''

if old_normal in cli:
    cli = cli.replace(old_normal, new_normal, 1)
    print("[OK] added to normal mode")

# Add to deep mode
old_deep = '''            "tls_checker", "subdomain_enum", "cve_lookup",
        ],
        "param_discovery_max": 300,'''
new_deep = '''            "tls_checker", "subdomain_enum", "cve_lookup", "endpoint_catalog",
        ],
        "param_discovery_max": 300,'''

if old_deep in cli:
    cli = cli.replace(old_deep, new_deep, 1)
    print("[OK] added to deep mode")
